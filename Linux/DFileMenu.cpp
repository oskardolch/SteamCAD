...
static void file_open_click(PDApplication pApp)
{
	/*GtkWidget *dialog = gtk_file_chooser_dialog_new(_("Open File"),
		wParent, GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL,
		GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
	//gtk_file_chooser_set_select_multiple((GtkFileChooser*)dialog, TRUE);
	GtkFileFilter *flt = gtk_file_filter_new();
	gtk_file_filter_set_name(flt, _("Oskar Dolch Files"));
	gtk_file_filter_add_pattern(flt, "*.od");
	//gtk_file_filter_add_mime_type(flt, "video/mpeg");
	gtk_file_chooser_set_filter((GtkFileChooser*)dialog, flt);
	gtk_file_chooser_set_current_folder((GtkFileChooser*)dialog, gLastInputPath);

	if(gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
	{
		g_free(gLastInputPath);
		gLastInputPath = gtk_file_chooser_get_current_folder((GtkFileChooser*)dialog);
	}
...
static void file_save_click(PDApplication pApp)
{
	...
	/*GtkWidget *dialog = gtk_file_chooser_dialog_new(_("Save File"),
		wParent, GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL,
		GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_ACCEPT, NULL);
	g_object_set(dialog, "do_overwrite_confirmation", TRUE, NULL);
	gtk_file_chooser_set_current_folder((GtkFileChooser*)dialog, gLastOutputPath);

	if(gtk_dialog_run(GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT)
	{
		g_free(gLastOutputPath);
		gLastOutputPath = gtk_file_chooser_get_current_folder((GtkFileChooser*)dialog);
	}
	gtk_widget_destroy(dialog);*/
