...
void DL_WriterA::dxfInt(int gc, int value) const {
    ...
    {
        char buf[64];
        if(gc < 10) sprintf(buf, "  %d\n%d\n", gc, value);
        else if(gc < 100) sprintf(buf, " %d\n%d\n", gc, value);
        else sprintf(buf, " %d\n%d\n", gc, value);
        fwrite(buf, 1, strlen(buf), m_pf);
    }
    else
    {
...
void DL_WriterA::dxfString(int gc, const char* value) const {
    ...
    if(m_pf)
    {
        ...
        fwrite(buf, 1, strlen(buf), m_pf);
        fwrite(value, 1, strlen(value), m_pf);
        fwrite("\n", 1, strlen("\n"), m_pf);
    }
    else
    {
...
void DL_WriterA::dxfString(int gc, const std::string& value) const {
    if(m_pf)
    {
        ...
        fwrite(buf, 1, strlen(buf), m_pf);
        fwrite(value.data(), 1, value.size(), m_pf);
        fwrite("\n", 1, strlen("\n"), m_pf);
    }
    else
    {
